...
async function clRegisterPispEndpoints(pispId, callbackUrl) {
  const endpoints = [
    ...
    { type: 'TP_CB_URL_CONSENT_POST', value: `${callbackUrl}/consents` },
    { type: 'TP_CB_URL_CONSENT_PUT', value: `${callbackUrl}/consents/{{ID}}` },
    {
    ...
    { type: 'TP_CB_URL_ACCOUNTS_GET', value: `${callbackUrl}/accounts/{{ID}}` },
    { type: 'TP_CB_URL_ACCOUNTS_PUT', value: `${callbackUrl}/accounts/{{ID}}` },
    {
...
exports.getPisps = async (req, res) => {
  try {
    const [rows] = await pool.execute(
      `SELECT * FROM pisps ORDER BY created_at DESC`,
    );
    res.json({ data: rows });
  } catch (err) {
...
exports.getPispById = async (req, res) => {
  try {
    ...
    const [[pisp]] = await pool.execute(
      `SELECT * FROM pisps WHERE pisp_id = ?`,
      [pispId],
    );
...
exports.createPisp = async (req, res) => {
  ...
  if (!pisp_id || !name || !callback_url)
    return res
      .status(400)
      .json({ error: 'pisp_id, name, callback_url required' });
    ...
    res.json({
      message:
        failed.length === 0
          ...
          : `${failed.length} endpoints failed`,
      results,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
