...
exports.getReconciliation = async (req, res) => {
  try {
    ...
    if (dfsp_id) {
      conditions.push(`r.dfsp_id = ?`);
      values.push(dfsp_id);
    }
    if (recon_status) {
      conditions.push(`r.recon_status = ?`);
      values.push(recon_status);
    }
    if (from) {
    ...
    const [[{ total }]] = await pool.execute(
      `SELECT COUNT(*) as total FROM reconciliation r ${where}`,
      values,
    );
