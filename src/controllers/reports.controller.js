...
function buildFilters(query) {
  ...
  if (date_preset === 'today') {
    ...
    conditions.push(
      `t.created_at >= DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)`,
    );
  } else if (from && to) {
...
exports.exportExcel = async (req, res) => {
  try {
    ...
    summary_sheet.columns = [{ width: 30 }, { width: 25 }, { width: 20 }];
    ...
    titleCell.font = {
      bold: true,
      size: 16,
      color: { argb: 'FF' + C.headerFg },
      name: 'Arial',
    };
    titleCell.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: 'FF' + C.headerBg },
    };
    ...
    statsHeader.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: 'FF' + C.accentBg.replace('#', '') },
    };
    ...
    const statsData = [
      ...
      [
        'Committed',
        s.committed,
        `${s.total > 0 ? ((s.committed / s.total) * 100).toFixed(1) : 0}% success rate`,
      ],
      ['Reserved', s.reserved, ''],
      ['Prepared', s.prepared, ''],
      [
        'Failed',
        s.failed,
        `${s.total > 0 ? ((s.failed / s.total) * 100).toFixed(1) : 0}% failure rate`,
      ],
      ['Timeout', s.timeout, ''],
      [
        'Total Amount',
        parseFloat(s.total_amount || 0).toFixed(2),
        'Committed transfers only',
      ],
      [
        'Avg Duration',
        s.avg_duration ? `${parseFloat(s.avg_duration).toFixed(1)}s` : 'N/A',
        'Committed transfers',
      ],
    ];
